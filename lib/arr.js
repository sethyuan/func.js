...
function map(f) {
  ...
  var res = new Array(shortestLen);
  for (var i = 0; i < res.length; i++) {
    res[i] = f.apply(undefined, _toConsumableArray(nth(i)));
  }
  return res;
}
...
function mapcat(f) {
  ...
  return concat.apply(undefined, _toConsumableArray(map.apply(undefined, [f].concat(arrs))));
}
